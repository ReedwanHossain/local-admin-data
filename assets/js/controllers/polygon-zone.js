(function() {
  ...
  angular.module("barikoi").controller("PolygonZone", PolygonZone);
  ...
  function PolygonZone(
    ...
    Auth.getlocations(
      ...
      function(res) {
        $scope.selectedItems = [];
        $scope.zones = [];
        $scope.zones.push({ key: -1, name: "clear", pgon: "nai", selected: false });
        res.map(function(res) {
          var polyjson = res["ST_AsGeoJSON(zone_area)"];
          $scope.zones.push({ key: res.id, name: res.id, pgon: polyjson, selected: false });
        });
