(function() {
  ...
  angular.module("barikoi").controller("PolygonWard", PolygonWard);
  ...
  function PolygonWard(
    ...
    $scope.set_zone = function(area) {
      ...
      Auth.getlocations(
        urls.WARDS_BY_ZONE + $scope.id + "?",
        function(res) {
          // $scope.subareas = [{key:1, name: 'Mirpur 2'}];
          $scope.wards = [];
          $scope.wards.push({ key: -1, name: "clear", pgon: "nai", selected: false });
          res.map(function(res) {
            var polyjson = res["ST_AsGeoJSON(ward_area)"];
            $scope.wards.push({
              key: res.id,
              name: res.ward_number,
              pgon: polyjson,
              selected: false
            });
          });
