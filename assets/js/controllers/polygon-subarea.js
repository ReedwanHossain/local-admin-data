(function() {
  ...
  angular.module("barikoi").controller("PolygonSubArea", PolygonSubArea);

  PolygonSubArea.$inject = [
    "$scope",
    "$http",
    "$stateParams",
    "$window",
  ...
  function PolygonSubArea(
    ...
    $scope.set_area = function(area) {
      ...
      Auth.getlocations(
        urls.SUBAREA_BY_AREA + $scope.id + "?",
        function(res) {
          // $scope.subareas = [{key:1, name: 'Mirpur 2'}];
          $scope.subareas = [];
          $scope.subareas.push({ key: -1, name: "clear", pgon: "nai", selected: false });
          res.map(function(res) {
            var polyjson = res["ST_AsGeoJSON(subarea_area)"];
            $scope.subareas.push({
              key: res.id,
              name: res.subarea_name,
              pgon: polyjson,
              selected: false
            });
          });
